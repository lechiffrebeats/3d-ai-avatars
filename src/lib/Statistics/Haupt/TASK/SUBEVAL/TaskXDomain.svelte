<script lang="ts">
  ...
  const demo_ai_arr =
    $statistics.demographie?.aggregated?.demo_domain_familiarity?.values;
  ...
  const taskCorrectnessMap = Object.values($statistics.tasks.ab_metrics)
  ...
  $: scatterData = Object.entries(taskCorrectnessMap).map(([sid, d]: any) => [
    d.domFamiliarity ?? 0,
    d.numCorrect ?? 0,
    sid,
  ]);
...
<Container
  title="Task Correctness x Domain Familiarity"
  description="Sample Size: {scatterData.length}, averages"
>
